...
class Release {
  ...
  getBuild (platform, arch) {
    const requiredName = `otel-cli_${this.version}_${platform}_${arch}.zip`;
...
async function fetchReleases () {
  const url = 'https://api.github.com/repos/equinix-labs/otel-cli/releases';
...
async function getRelease (version, fetchReleasesFn = fetchReleases) {
  ...
  if (releases === null || releases.length === 0) {
    throw new Error('No otel-cli releases found, please contact otel-cli');
  }
